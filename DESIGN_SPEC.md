...
main.py
  ...
  -> initializes DB schema and sample data
...
| `CALENDAR_ACCESS_TOKEN` | Yes for ICS | none | Token required by calendar download endpoint |
| `GOOGLE_TOKEN_DIR` | No | `./data/google_tokens` | Google OAuth token directory |
...
- `.env`, DB files, and Google OAuth token files must not be committed.
